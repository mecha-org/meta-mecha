# Auto-Generated by cargo-bitbake 0.3.16
#
inherit cargo
# If this is git based prefer versioned ones if they exist
# DEFAULT_PREFERENCE = "-1"
# how to get mecha_agent_server could be as easy as but default to a git checkout:
# SRC_URI += "crate://crates.io/mecha_agent_server/1.0.0"
SRC_URI += "git://git@github.com/dhruveshb-mecha/mecha-agent.git;protocol=ssh;nobranch=1;branch=dev-v1"
SRCREV = "e4cbee927849b19d02284cdffdcf0a1308879856"
S = "${WORKDIR}/git"
CARGO_SRC_DIR = "server"
PV:append = ".AUTOINC+e4cbee9278"
# please note if you have entries that do not begin with crate://
# you must change them to how that package can be fetched
SRC_URI += " \
    crate://crates.io/addr2line/0.21.0 \
    crate://crates.io/adler/1.0.2 \
    crate://crates.io/aho-corasick/1.0.5 \
    crate://crates.io/android-tzdata/0.1.1 \
    crate://crates.io/android_system_properties/0.1.5 \
    crate://crates.io/anyhow/1.0.75 \
    crate://crates.io/async-nats/0.31.0 \
    crate://crates.io/async-stream-impl/0.3.5 \
    crate://crates.io/async-stream/0.3.5 \
    crate://crates.io/async-trait/0.1.73 \
    crate://crates.io/autocfg/1.1.0 \
    crate://crates.io/axum-core/0.3.4 \
    crate://crates.io/axum-macros/0.3.8 \
    crate://crates.io/axum-tracing-opentelemetry/0.13.1 \
    crate://crates.io/axum/0.6.20 \
    crate://crates.io/backtrace/0.3.69 \
    crate://crates.io/base64/0.13.1 \
    crate://crates.io/base64/0.21.4 \
    crate://crates.io/base64ct/1.6.0 \
    crate://crates.io/bincode/1.3.3 \
    crate://crates.io/bitflags/1.3.2 \
    crate://crates.io/bitflags/2.4.0 \
    crate://crates.io/block-buffer/0.10.4 \
    crate://crates.io/bumpalo/3.13.0 \
    crate://crates.io/byteorder/1.4.3 \
    crate://crates.io/bytes/1.5.0 \
    crate://crates.io/cc/1.0.83 \
    crate://crates.io/cfg-if/1.0.0 \
    crate://crates.io/chrono/0.4.31 \
    crate://crates.io/const-oid/0.9.5 \
    crate://crates.io/cookie/0.18.0 \
    crate://crates.io/core-foundation-sys/0.8.4 \
    crate://crates.io/core-foundation/0.9.3 \
    crate://crates.io/cpufeatures/0.2.9 \
    crate://crates.io/crc32fast/1.3.2 \
    crate://crates.io/crossbeam-channel/0.5.8 \
    crate://crates.io/crossbeam-epoch/0.9.15 \
    crate://crates.io/crossbeam-utils/0.8.16 \
    crate://crates.io/crypto-common/0.1.6 \
    crate://crates.io/curve25519-dalek-derive/0.1.0 \
    crate://crates.io/curve25519-dalek/4.1.0 \
    crate://crates.io/dashmap/5.5.3 \
    crate://crates.io/data-encoding/2.4.0 \
    crate://crates.io/debugid/0.8.0 \
    crate://crates.io/der/0.7.8 \
    crate://crates.io/deranged/0.3.8 \
    crate://crates.io/digest/0.10.7 \
    crate://crates.io/dotenv/0.15.0 \
    crate://crates.io/ed25519-dalek/2.0.0 \
    crate://crates.io/ed25519/2.2.2 \
    crate://crates.io/either/1.9.0 \
    crate://crates.io/encoding_rs/0.8.33 \
    crate://crates.io/equivalent/1.0.1 \
    crate://crates.io/errno-dragonfly/0.1.2 \
    crate://crates.io/errno/0.3.3 \
    crate://crates.io/fastrand/2.0.0 \
    crate://crates.io/fiat-crypto/0.2.1 \
    crate://crates.io/findshlibs/0.10.2 \
    crate://crates.io/fixedbitset/0.4.2 \
    crate://crates.io/fnv/1.0.7 \
    crate://crates.io/foreign-types-shared/0.1.1 \
    crate://crates.io/foreign-types/0.3.2 \
    crate://crates.io/form_urlencoded/1.2.0 \
    crate://crates.io/fs2/0.4.3 \
    crate://crates.io/futures-channel/0.3.28 \
    crate://crates.io/futures-core/0.3.28 \
    crate://crates.io/futures-executor/0.3.28 \
    crate://crates.io/futures-io/0.3.28 \
    crate://crates.io/futures-macro/0.3.28 \
    crate://crates.io/futures-sink/0.3.28 \
    crate://crates.io/futures-task/0.3.28 \
    crate://crates.io/futures-util/0.3.28 \
    crate://crates.io/futures/0.3.28 \
    crate://crates.io/fxhash/0.2.1 \
    crate://crates.io/generic-array/0.14.7 \
    crate://crates.io/getrandom/0.2.10 \
    crate://crates.io/gimli/0.28.0 \
    crate://crates.io/h2/0.3.21 \
    crate://crates.io/hashbrown/0.12.3 \
    crate://crates.io/hashbrown/0.14.0 \
    crate://crates.io/headers-core/0.2.0 \
    crate://crates.io/headers/0.3.9 \
    crate://crates.io/heck/0.4.1 \
    crate://crates.io/hermit-abi/0.3.2 \
    crate://crates.io/hex/0.4.3 \
    crate://crates.io/home/0.5.5 \
    crate://crates.io/hostname/0.3.1 \
    crate://crates.io/http-body/0.4.5 \
    crate://crates.io/http-range-header/0.3.1 \
    crate://crates.io/http/0.2.9 \
    crate://crates.io/httparse/1.8.0 \
    crate://crates.io/httpdate/1.0.3 \
    crate://crates.io/hyper-timeout/0.4.1 \
    crate://crates.io/hyper-tls/0.5.0 \
    crate://crates.io/hyper/0.14.27 \
    crate://crates.io/hyper_serde/0.13.2 \
    crate://crates.io/iana-time-zone-haiku/0.1.2 \
    crate://crates.io/iana-time-zone/0.1.57 \
    crate://crates.io/idna/0.4.0 \
    crate://crates.io/indexmap/1.9.3 \
    crate://crates.io/indexmap/2.0.0 \
    crate://crates.io/init-tracing-opentelemetry/0.12.0 \
    crate://crates.io/instant/0.1.12 \
    crate://crates.io/ipnet/2.8.0 \
    crate://crates.io/itertools/0.10.5 \
    crate://crates.io/itertools/0.9.0 \
    crate://crates.io/itoa/1.0.9 \
    crate://crates.io/js-sys/0.3.64 \
    crate://crates.io/lazy_static/1.4.0 \
    crate://crates.io/libc/0.2.147 \
    crate://crates.io/linux-raw-sys/0.4.7 \
    crate://crates.io/lock_api/0.4.10 \
    crate://crates.io/log/0.4.20 \
    crate://crates.io/match_cfg/0.1.0 \
    crate://crates.io/matchers/0.1.0 \
    crate://crates.io/matchit/0.7.2 \
    crate://crates.io/memchr/2.6.3 \
    crate://crates.io/memoffset/0.9.0 \
    crate://crates.io/mime/0.3.17 \
    crate://crates.io/miniz_oxide/0.7.1 \
    crate://crates.io/mio/0.8.8 \
    crate://crates.io/mkdirp/1.0.0 \
    crate://crates.io/multimap/0.8.3 \
    crate://crates.io/native-tls/0.2.11 \
    crate://crates.io/nkeys/0.3.2 \
    crate://crates.io/nu-ansi-term/0.46.0 \
    crate://crates.io/nuid/0.3.2 \
    crate://crates.io/num-traits/0.2.16 \
    crate://crates.io/num_cpus/1.16.0 \
    crate://crates.io/object/0.32.1 \
    crate://crates.io/once_cell/1.18.0 \
    crate://crates.io/openssl-macros/0.1.1 \
    crate://crates.io/openssl-probe/0.1.5 \
    crate://crates.io/openssl-sys/0.9.93 \
    crate://crates.io/openssl/0.10.57 \
    crate://crates.io/opentelemetry-http/0.8.0 \
    crate://crates.io/opentelemetry-http/0.9.0 \
    crate://crates.io/opentelemetry-otlp/0.12.0 \
    crate://crates.io/opentelemetry-proto/0.2.0 \
    crate://crates.io/opentelemetry-semantic-conventions/0.11.0 \
    crate://crates.io/opentelemetry/0.19.0 \
    crate://crates.io/opentelemetry/0.20.0 \
    crate://crates.io/opentelemetry_api/0.19.0 \
    crate://crates.io/opentelemetry_api/0.20.0 \
    crate://crates.io/opentelemetry_sdk/0.19.0 \
    crate://crates.io/opentelemetry_sdk/0.20.0 \
    crate://crates.io/ordered-float/3.9.1 \
    crate://crates.io/os_info/3.7.0 \
    crate://crates.io/overload/0.1.1 \
    crate://crates.io/parking_lot/0.11.2 \
    crate://crates.io/parking_lot/0.12.1 \
    crate://crates.io/parking_lot_core/0.8.6 \
    crate://crates.io/parking_lot_core/0.9.8 \
    crate://crates.io/pem-rfc7468/0.7.0 \
    crate://crates.io/percent-encoding/2.3.0 \
    crate://crates.io/petgraph/0.6.4 \
    crate://crates.io/pin-project-internal/1.1.3 \
    crate://crates.io/pin-project-lite/0.2.13 \
    crate://crates.io/pin-project/1.1.3 \
    crate://crates.io/pin-utils/0.1.0 \
    crate://crates.io/pkcs8/0.10.2 \
    crate://crates.io/pkg-config/0.3.27 \
    crate://crates.io/platforms/3.1.2 \
    crate://crates.io/ppv-lite86/0.2.17 \
    crate://crates.io/prettyplease/0.1.25 \
    crate://crates.io/proc-macro2/1.0.66 \
    crate://crates.io/prost-build/0.11.9 \
    crate://crates.io/prost-derive/0.11.9 \
    crate://crates.io/prost-derive/0.7.0 \
    crate://crates.io/prost-types/0.11.9 \
    crate://crates.io/prost-types/0.7.0 \
    crate://crates.io/prost/0.11.9 \
    crate://crates.io/prost/0.7.0 \
    crate://crates.io/protobuf-support/3.2.0 \
    crate://crates.io/protobuf/3.2.0 \
    crate://crates.io/quote/1.0.33 \
    crate://crates.io/rand/0.8.5 \
    crate://crates.io/rand_chacha/0.3.1 \
    crate://crates.io/rand_core/0.6.4 \
    crate://crates.io/redox_syscall/0.2.16 \
    crate://crates.io/redox_syscall/0.3.5 \
    crate://crates.io/regex-automata/0.1.10 \
    crate://crates.io/regex-automata/0.3.8 \
    crate://crates.io/regex-syntax/0.6.29 \
    crate://crates.io/regex-syntax/0.7.5 \
    crate://crates.io/regex/1.9.5 \
    crate://crates.io/reqwest/0.11.20 \
    crate://crates.io/ring/0.16.20 \
    crate://crates.io/rustc-demangle/0.1.23 \
    crate://crates.io/rustc_version/0.4.0 \
    crate://crates.io/rustix/0.38.13 \
    crate://crates.io/rustls-native-certs/0.6.3 \
    crate://crates.io/rustls-pemfile/1.0.3 \
    crate://crates.io/rustls-webpki/0.101.5 \
    crate://crates.io/rustls/0.21.7 \
    crate://crates.io/rustversion/1.0.14 \
    crate://crates.io/ryu/1.0.15 \
    crate://crates.io/schannel/0.1.22 \
    crate://crates.io/scopeguard/1.2.0 \
    crate://crates.io/sct/0.7.0 \
    crate://crates.io/security-framework-sys/2.9.1 \
    crate://crates.io/security-framework/2.9.2 \
    crate://crates.io/semver/1.0.18 \
    crate://crates.io/sentry-anyhow/0.31.6 \
    crate://crates.io/sentry-backtrace/0.31.6 \
    crate://crates.io/sentry-contexts/0.31.6 \
    crate://crates.io/sentry-core/0.31.6 \
    crate://crates.io/sentry-debug-images/0.31.6 \
    crate://crates.io/sentry-panic/0.31.6 \
    crate://crates.io/sentry-tracing/0.31.6 \
    crate://crates.io/sentry-types/0.31.6 \
    crate://crates.io/sentry/0.31.6 \
    crate://crates.io/serde/1.0.188 \
    crate://crates.io/serde_bytes/0.11.12 \
    crate://crates.io/serde_derive/1.0.188 \
    crate://crates.io/serde_json/1.0.106 \
    crate://crates.io/serde_nanos/0.1.3 \
    crate://crates.io/serde_path_to_error/0.1.14 \
    crate://crates.io/serde_repr/0.1.16 \
    crate://crates.io/serde_urlencoded/0.7.1 \
    crate://crates.io/serde_yaml/0.9.25 \
    crate://crates.io/sha1/0.10.6 \
    crate://crates.io/sha2/0.10.7 \
    crate://crates.io/sha256/1.4.0 \
    crate://crates.io/sharded-slab/0.1.4 \
    crate://crates.io/signal-hook-registry/1.4.1 \
    crate://crates.io/signatory/0.27.1 \
    crate://crates.io/signature/2.0.0 \
    crate://crates.io/slab/0.4.9 \
    crate://crates.io/sled/0.34.7 \
    crate://crates.io/smallvec/1.11.0 \
    crate://crates.io/socket2/0.4.9 \
    crate://crates.io/socket2/0.5.4 \
    crate://crates.io/spin/0.5.2 \
    crate://crates.io/spki/0.7.2 \
    crate://crates.io/subtle/2.5.0 \
    crate://crates.io/syn/1.0.109 \
    crate://crates.io/syn/2.0.32 \
    crate://crates.io/sync_wrapper/0.1.2 \
    crate://crates.io/tempfile/3.8.0 \
    crate://crates.io/thiserror-impl/1.0.48 \
    crate://crates.io/thiserror/1.0.48 \
    crate://crates.io/thread_local/1.1.7 \
    crate://crates.io/time-core/0.1.1 \
    crate://crates.io/time-macros/0.2.14 \
    crate://crates.io/time/0.1.45 \
    crate://crates.io/time/0.3.28 \
    crate://crates.io/tinyvec/1.6.0 \
    crate://crates.io/tinyvec_macros/0.1.1 \
    crate://crates.io/tokio-io-timeout/1.2.0 \
    crate://crates.io/tokio-macros/2.1.0 \
    crate://crates.io/tokio-native-tls/0.3.1 \
    crate://crates.io/tokio-retry/0.3.0 \
    crate://crates.io/tokio-rustls/0.24.1 \
    crate://crates.io/tokio-stream/0.1.14 \
    crate://crates.io/tokio-util/0.7.8 \
    crate://crates.io/tokio/1.32.0 \
    crate://crates.io/tonic-build/0.9.2 \
    crate://crates.io/tonic-types/0.9.2 \
    crate://crates.io/tonic/0.8.3 \
    crate://crates.io/tonic/0.9.2 \
    crate://crates.io/tower-http/0.3.5 \
    crate://crates.io/tower-layer/0.3.2 \
    crate://crates.io/tower-service/0.3.2 \
    crate://crates.io/tower/0.4.13 \
    crate://crates.io/tracing-attributes/0.1.26 \
    crate://crates.io/tracing-core/0.1.31 \
    crate://crates.io/tracing-futures/0.2.5 \
    crate://crates.io/tracing-log/0.1.3 \
    crate://crates.io/tracing-opentelemetry-instrumentation-sdk/0.12.0 \
    crate://crates.io/tracing-opentelemetry-instrumentation-sdk/0.13.1 \
    crate://crates.io/tracing-opentelemetry/0.19.0 \
    crate://crates.io/tracing-opentelemetry/0.20.0 \
    crate://crates.io/tracing-serde/0.1.3 \
    crate://crates.io/tracing-subscriber/0.3.17 \
    crate://crates.io/tracing/0.1.37 \
    crate://crates.io/try-lock/0.2.4 \
    crate://crates.io/typenum/1.17.0 \
    crate://crates.io/uname/0.1.1 \
    crate://crates.io/unicode-bidi/0.3.13 \
    crate://crates.io/unicode-ident/1.0.11 \
    crate://crates.io/unicode-normalization/0.1.22 \
    crate://crates.io/unsafe-libyaml/0.2.9 \
    crate://crates.io/untrusted/0.7.1 \
    crate://crates.io/ureq/2.7.1 \
    crate://crates.io/url/2.4.1 \
    crate://crates.io/urlencoding/2.1.3 \
    crate://crates.io/uuid-macro-internal/1.4.1 \
    crate://crates.io/uuid/1.4.1 \
    crate://crates.io/valuable/0.1.0 \
    crate://crates.io/vcpkg/0.2.15 \
    crate://crates.io/version_check/0.9.4 \
    crate://crates.io/want/0.3.1 \
    crate://crates.io/wasi/0.10.0+wasi-snapshot-preview1 \
    crate://crates.io/wasi/0.11.0+wasi-snapshot-preview1 \
    crate://crates.io/wasm-bindgen-backend/0.2.87 \
    crate://crates.io/wasm-bindgen-futures/0.4.37 \
    crate://crates.io/wasm-bindgen-macro-support/0.2.87 \
    crate://crates.io/wasm-bindgen-macro/0.2.87 \
    crate://crates.io/wasm-bindgen-shared/0.2.87 \
    crate://crates.io/wasm-bindgen/0.2.87 \
    crate://crates.io/web-sys/0.3.64 \
    crate://crates.io/which/4.4.2 \
    crate://crates.io/winapi-i686-pc-windows-gnu/0.4.0 \
    crate://crates.io/winapi-x86_64-pc-windows-gnu/0.4.0 \
    crate://crates.io/winapi/0.3.9 \
    crate://crates.io/windows-sys/0.48.0 \
    crate://crates.io/windows-targets/0.48.5 \
    crate://crates.io/windows/0.48.0 \
    crate://crates.io/windows_aarch64_gnullvm/0.48.5 \
    crate://crates.io/windows_aarch64_msvc/0.48.5 \
    crate://crates.io/windows_i686_gnu/0.48.5 \
    crate://crates.io/windows_i686_msvc/0.48.5 \
    crate://crates.io/windows_x86_64_gnu/0.48.5 \
    crate://crates.io/windows_x86_64_gnullvm/0.48.5 \
    crate://crates.io/windows_x86_64_msvc/0.48.5 \
    crate://crates.io/winreg/0.50.0 \
    crate://crates.io/zeroize/1.6.0 \
"
DEPENDS:append = " openssl pkgconfig pkgconfig-native udev wayland wayland-utils grpc nativesdk-protobuf protobuf-c-native"

LICENSE="MIT"
LIC_FILES_CHKSUM = "file://${COMMON_LICENSE_DIR}/MIT;md5=0835ade698e0bcf8506ecda2f7b4f302"


SUMMARY = "Mecha Agent on the device for provisioning, messaging and more"
HOMEPAGE = "https://mecha.so"
LICENSE = "MIT"
# includes this file if it exists but does not fail
# this is useful for anything you may want to override from
# what cargo-bitbake generates.
include mecha_agent_server-${PV}.inc
include mecha_agent_server.inc
